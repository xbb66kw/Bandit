...
def bandit_low(data, residuals, coef):
				...
				for k in range(len_):
								if int(np.sqrt(k)) != np.sqrt(k) and k >  K * np.sqrt(len_coef):#int(k / 10) != k / 10: #int(np.sqrt(k)) != np.sqrt(k):
...
def bandit_high(data, residuals, coef):
				...
				for k in range(len_):
								if int(np.sqrt(k)) != np.sqrt(k) and k >  K * np.sqrt(len_coef):
												weights = choice_calculator(estimates, data_[k,:], beta, K, p / K)
...
def bandit_bylearning(data, residuals, coef, n_algo=2):
				...
				for k in range(len_):
								if k != 0 and not np.sqrt(k).is_integer() and k >  K * np.sqrt(len_coef):
