...
class Collector(object):
				...
				def model_selection_graphs(self, list_graphs):
								...
								if len(articles) > 0:
												...
												for article in articles:
																...
																X, y = self.get_article_covariates(article, keys_set - second_graph_second_key_set)
																...
																list_key_sets = [keys_set - second_graph_second_key_set - second_graph_extra_key_set, second_graph_extra_key_set - second_graph_second_key_set, second_graph_second_key_set]
																list_key_sets_2 = []
												...
												####For the first graph
												X, y = self.get_article_covariates(graph.get_articles - articles, keys_set)
												...
												'''Make sure there's no need for classification anymore'''
												X, y = self.get_article_covariates(graph.get_articles - articles, keys_set - first_graph_second_key_set)
												
												'''Combo!''' 
												ones, zeros = Util.zeros_and_ones(Logistic.model_, Logistic.coef_)
												first_graph_extra_key_set = self.get_sets(ones, zeros) #keys set
												
												list_key_sets = [keys_set - first_graph_second_key_set - first_graph_extra_key_set, first_graph_extra_key_set - first_graph_second_key_set, first_graph_second_key_set]
												list_key_sets_1 = []
												...
												graph1 = Graph(graph.get_articles - articles, list_key_sets_1, self)#collector
												...
												X, y = self.get_article_covariates(graph.get_articles, keys_set)
												...
												if len(extra_key_set) == len(keys_set) or len(extra_key_set) == 0:                
																final_graphs_list.append(Graph(graph.get_articles, [keys_set], self))
																...
																final_graphs_list.append(Graph(graph.get_articles, list_keys, self))#collector				
				...
				####
								for	graph in list_graphs[1:]:						
												...
												graph2 = Graph(article, list_key_sets_2, self)#collector
