...
class model_UCB1(object):
				...
				def recommend(self, key, cov):
								...
								if not key in whole_key:												
												...
												max_index = model_UCB1.max_finder(numbers, clicks, self.counter, 0.15)
												self.last_action = articles_[max_index]
												
												return self.last_action
								
								
								#Two clustering UCB have distinguished parameters
								if len(self.__list_graphs) == 2 and len(self.__list_graphs[1].get_articles) > 1:
												cluster_finder = model_UCB1.max_finder_expedience
								else:
												cluster_finder = model_UCB1.max_finder_cluster
								...
								max_index = cluster_finder(cov, self.__list_graphs, numbers, clicks, self.counter, self.alpha, self.__list_graphs[0].model)
								#print(numbers, clicks, max_index, '339')
								#self.first_round = max_index #TESTING
								
								if max_index == 0:
												#articles = []
												#articles.extend(self.__list_graphs[0].get_articles)
												#self.last_action = articles[np.random.choice(len(articles), 1)[0]]
												...
												#expedience is of different exploration parameter
												max_index = model_UCB1.max_finder_expedience(cov, self.__list_graphs[max_index], numbers, clicks, self.counter, self.alpha, self.__list_graphs[max_index].model)
												self.last_action = articles_[max_index]
												...
												max_index = model_UCB1.max_finder_within(numbers, clicks, self.counter, self.alpha)
												self.last_action = articles_[max_index]
				...
				@staticmethod
				def max_finder(numbers, clicks, total = None, alpha = None):
								'''
								numbers: list
								clicks: list
								'''
								...
								array_ = (clicks) / (numbers + 1) + alpha * np.sqrt(np.log(sum_+1)/(numbers+1))								
				...
				def max_finder_cluster(cov, graphs, numbers, clicks, total = None, alpha = None, model=None):
								...
								if model is None:
												array_ = (clicks)/ (numbers + 1) + 0.7 * np.sqrt(np.log(sum_+1)/(numbers+1))
								else:
												directions = []
												sub_cov = cov[model]
												for graph in graphs:
																inv = graph.get_overall_inverse
																directions.append(np.dot(np.dot(sub_cov.T, inv), sub_cov))
																
												directions = np.array(directions)
												array_ = (clicks)/ (numbers + 1) + 0.07 * np.sqrt(np.log(sum_+1)/(numbers+1)) + directions
												#print(directions, '417')
								return np.random.choice(np.where(array_ == array_.max())[0])
				...
				@staticmethod
				def max_finder_within(numbers, clicks, total = None, alpha = None):
								'''
								numbers: list
								clicks: list
								'''
								...
								clicks = np.array(clicks)
								###ALPHA!!
								mid_ = np.c_[clicks-1, np.zeros(len(clicks))]
								array_ = (clicks) / (numbers + 1) + 0.07 * np.sqrt(np.log(sum_+1)/(numbers+1))							
				...
				def max_finder_expedience(cov, graph, numbers, clicks, total = None, alpha = None, model=None):
								...
								clicks = np.array(clicks)
								if model is None:
												array_ = (clicks)/ (numbers + 1) + 0.18 * np.sqrt(np.log(sum_+1)/(numbers+1))
								else:
												directions = []
												sub_cov = cov[model]
												dictionary = graph.get_article_inverses
												loop_articles = dictionary.keys()
												#print(len(numbers), len(loop_articles), '509')
												for article in loop_articles:																
																inv = dictionary[article]
																directions.append(np.dot(np.dot(sub_cov.T, inv), sub_cov))
																
												directions = np.array(directions)
												array_ = (clicks)/ (numbers + 1) + 0.18 * np.sqrt(np.log(sum_+1)/(numbers+1)) + directions
												
												#print(directions, '514')
								return np.random.choice(np.where(array_ == array_.max())[0])				
...
class Agent_model_UCB1(object):
    ...
    def recommend(self, stuff):
        ...
        self.last_action = self.model_UCB1_object.recommend(key, covariates) #self.last_action is a number         
