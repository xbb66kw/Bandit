...
class model_UCB1(object):
    ...
    def max_finder(numbers, clicks):
        ...
        ###ALPHA!!
        return np.argmax(clicks / (numbers + 1) + 0.05 * np.sqrt(np.log(sum_)/(numbers+1)))
    ...
    def model_selection_graphs(self, list_graphs):
								...
								for key_set_in_the_list in list_of_keys_sets:
												...
												if len(y) > 5:
																...
																'''Combo!''' 
																ones, zeros = Util.zeros_and_ones(Logistic.model_, Logistic.coef_)
																second_key_set = self.get_sets(ones, zeros, key_set_in_the_list) #keys set
																
																
																
																if len(key_set_in_the_list) != len(second_key_set) and len(second_key_set) != 0:
																				list_key_sets = [key_set_in_the_list - second_key_set, second_key_set]
																else: 
																				list_key_sets = [key_set_in_the_list]
																...
																list_.append(key_set_in_the_list)
...
class Agent_model_UCB1(object):
    ...
    def recommend(self, stuff):
        ...
        return self.last_action#!/usr/bin/env python
