...
class DataReader(object):
    def __init__(self):
        ...
        self.files_list = ['/Users/xbb/Desktop/bandit_experiment/r6b_69k.txt']
...
class model_UCB1(object):
    ...
    def recommend(self, key, cov):
        ...
        #special event
        if sum(cov[np.array(model_).astype(int)]) == len(np.array(model_)) or sum(cov[np.array(model_).astype(int)]) > 5:
            
            if int(self.counter / 500) == self.counter / 500:
                #print('number of special assignment:', self.counter_special, self.articles_counter, '372')
                print('hi------', ARTICLES_COUNTER, '373')
                print('hi', ARTICLES_COUNTER.keys(), '374')
                print('all article now', self.__articles_collector.active_articles, '374')
                pass
            
            for article in ARTICLES_COUNTER.keys():                                                
                if ARTICLES_COUNTER[article] <= GREEDY:                                                                                          
                    self.last_action = article
                    if int(self.counter / 2000) == self.counter / 2000:
                        #print('number of special assignment:', self.counter_special, self.articles_counter, '372')
                        print('hihi', ARTICLES_COUNTER, '380')
...
class Agent_model_UCB1(object):
    ...
    def update_arms(self, stuff):
        ...
        for article in stuff['extra']:
            if len(MODEL) > 4:
                ARTICLES_COUNTER[article] = 0
            else:
                ARTICLES_COUNTER[article] = int(GREEDY/2)
        for article in stuff['delete']:
            del ARTICLES_COUNTER[article]
...
class Environment(object):
    ...
    def plot(self, number_of_agents):
        ...
        for j in range(len(self.reward_curves[0,:])):
            ...
            mid_ = "/Users/xbb/Desktop/bandit_experiment/model_selection_clustering/r6b_59k_logistic" + str(j)
